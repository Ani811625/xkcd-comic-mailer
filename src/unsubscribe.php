...
// Handle unsubscribe request - step 1
if (isset($_POST['unsubscribe_email']) && !isset($_POST['verification_code'])) {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $message = "<div class='alert success'>Unsubscription code sent to $email</div>";
    } else {
        $message = "<div class='alert error'>Invalid email address.</div>";
    }
}

// Handle verification code to confirm unsubscription - step 2
if (isset($_POST['verification_code']) && isset($_POST['unsubscribe_email'])) {
    $email = trim($_POST['unsubscribe_email']);
    $code = trim($_POST['verification_code']);
    if (verifyCode($email, $code)) {
        unsubscribeEmail($email);
        $message = "<div class='alert success'>$email has been successfully unsubscribed!</div>";
    } else {
        $message = "<div class='alert error'>Unsubscription failed. Incorrect code.</div>";
    }
}
?>
<!DOCTYPE html>
    ...
    <div class="container">
        ...
        <form method="POST" autocomplete="off" style="margin-bottom: 24px; width:100%;">
            <div class="form-group">
                <input type="email" name="unsubscribe_email" id="email-unsub" required placeholder=" " autocomplete="email">
                <label for="email-unsub" class="floating-label">Email Address</label>
            </div>
            <button type="submit" id="submit-unsubscribe">Send Unsubscribe Code</button>
        </form>
        <h2 style="margin-top:24px;">Verify Unsubscription</h2>
        <form method="POST" autocomplete="off" style="width:100%;">
            <div class="form-group">
                <input type="text" name="verification_code" id="verification_code_unsub" maxlength="6" required placeholder=" ">
                <label for="verification_code_unsub" class="floating-label">Verification Code</label>
            </div>
            <div class="form-group">
                <input type="email" name="unsubscribe_email" id="email-unsub-ver" required placeholder=" " autocomplete="email">
                <label for="email-unsub-ver" class="floating-label">Email Address</label>
            </div>
